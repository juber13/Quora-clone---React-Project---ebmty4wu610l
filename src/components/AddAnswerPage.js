    ...
    <div className='container mt-5 py-4' style={container}>
     <div className="row">
      <div className='col-5  pt-10'>
        ...
        <ul>
          {questions.map((ques ,  index) => ques.status != true && <li onClick={() => updateQuestion(index)}key={index} className="select-question" role="button">{ques.text}</li>)}
        </ul>
      </div>
      <div className='col-7' >
      <label for="exampleFormControlTextarea1" className="form-label text-danger">Write Your Answer Here!!</label>
      <textarea required value={inputVal} onChange={(e) => setInputval(e.target.value)} className="form-control" placeholder={SelectedQuestion} id="exampleFormControlTextarea1" rows="15" cols="15"></textarea>  
      <Link to='/'>
      <button className='btn btn-danger m-2'>Cancel</button>
      </Link>
      <button className='btn btn-danger' onClick={addAnswer}>Add Answer</button>
      </div>
    </div>
    </div>
    <Footer/>
